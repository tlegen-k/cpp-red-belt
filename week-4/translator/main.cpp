...
class Translator {
public:
    Translator() : f_dict(), b_dict(), data()
    {}
    void Add(string_view source, string_view target)
    {
        const string_view source_view = GetClonedView(source);
        const string_view target_view = GetClonedView(target);
        f_dict[source_view] = target_view;
        b_dict[target_view] = source_view;

    }
    string_view TranslateForward(string_view source) const {
        return Translate(f_dict, source);
    }
    string_view TranslateBackward(string_view target) const {
        return Translate(b_dict, target);
    }
...
private:
    ...
    {
        for (const auto* map_ptr : {&f_dict, &b_dict}) {
    ...
    map<string_view, string_view> f_dict;
    map<string_view, string_view> b_dict;
    deque<string> data;
